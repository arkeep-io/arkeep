-- SQLite does not support altering CHECK constraints; the tables must be recreated.
-- PRAGMA foreign_keys = OFF is required because golang-migrate runs this migration
-- without a transaction (NoLock: true), allowing the PRAGMA to take effect.
...
CREATE TABLE job_destinations_new (
    ...
    CONSTRAINT job_destinations_status_check   CHECK (status IN ('pending', 'running', 'succeeded', 'failed', 'cancelled'))
);
